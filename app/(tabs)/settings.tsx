...
import { Trash2, Heart, Clock, Star, Share2, Info, ChevronRight, Database } from 'lucide-react-native';
import { colors } from '@/constants/colors';
import { usePickupStore } from '@/store/pickup-store';

export default function SettingsScreen() {
  const { favorites, recentlyViewed, userRatings, aiGeneratedLines } = usePickupStore();
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Clear', style: 'destructive', onPress: () => store.setState({ favorites: [] }) }
        ]
      );
    }
  };
  ...
  const clearHistory = () => {
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Clear', style: 'destructive', onPress: () => store.setState({ recentlyViewed: [] }) }
        ]
      );
    }
  };
  ...
  const clearRatings = () => {
    if (Platform.OS === 'web') {
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Clear', style: 'destructive', onPress: () => store.setState({ userRatings: {} }) }
        ]
      );
    }
  };
  ...
  const clearAICollection = () => {
    if (Platform.OS === 'web') {
      if (confirm('Are you sure you want to clear your AI collection?')) {
        store.setState({ aiGeneratedLines: [] });
      }
    } else {
      Alert.alert(
        'Clear AI Collection',
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Clear', style: 'destructive', onPress: () => store.setState({ aiGeneratedLines: [] }) }
        ]
      );
    }
  };
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statsContainer}>
            <View style={styles.statItem}>
              <View style={[styles.statIcon, { backgroundColor: colors.favorite + '20' }]}>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue}>{Object.keys(userRatings).length}</Text>
